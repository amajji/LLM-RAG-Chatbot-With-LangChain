...
def q_a_llm_model(temperature, max_new_tokens, top_p, top_k ):
    ...
    # Create a retriever object from the 'db' with a search configuration where it retrieves up to top_k relevant splits/documents.
    retriever = vector_db.as_retriever(search_kwargs={"k": top_k})

    # Create a question-answering instance (qa) using the RetrievalQA class.
    # It's configured with a language model (llm), a chain type "refine," the retriever we created, and an option to not return source documents.
    q_a = RetrievalQA.from_chain_type(llm=llm_model, chain_type="refine", retriever=retriever, return_source_documents=False)
...
def page_1():
    ...
    st.markdown(
        ...
        """This interactive dashboard is designed to extract any information from external documents. The LLM used is LLaMA2-7B with LangChain for RAG. The user has the possibility to ask questions and the LLM responds
        with answers from the available documents.
        To reduce memory needed to run the LLM, we've used quantization.
    ...
    if prompt := st.chat_input():
        ...
        st.session_state.messages.append({"role": "assistant", "content": "voici le message de retour"})
